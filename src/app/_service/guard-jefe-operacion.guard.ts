...
import { TOKEN_NAME, ROLE_JEFE_DISTRITO, ROLE_JEFE_OPERACION } from './var.const';
import * as decode from 'jwt-decode';

@Injectable()
export class GuardJefeOperacionGuard implements CanActivate {
  ...
  canActivate(next: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
    ...
    if (tokenNotExpired(TOKEN_NAME, token.access_token)) {
      ...
      if(role == ROLE_JEFE_DISTRITO || role == ROLE_JEFE_OPERACION) {
